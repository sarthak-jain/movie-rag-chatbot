...
I built and shipped my own AI movie chatbot 🎬 — and it cost me $0 to run.
...
🔹 FAISS vector index for semantic search — prebuilt offline so the free
   CPU tier never breaks a sweat
🔹 Llama 3 via Groq's free API for grounded, streamed answers
...
1️⃣ Local-only pieces (Ollama) don't survive deployment — swapping to a
   hosted free-tier LLM changed the architecture more than any other decision.
2️⃣ Pre-computing embeddings offline is the difference between "needs a GPU
   server" and "runs free forever."
